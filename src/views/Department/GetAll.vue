  ...
  <div id="data-list">
    <div class="vx-card p-6">
      <div class="flex flex-wrap items-center">
        ...
        <vs-button class="mb-4 md:mb-0" @click="goToAddDepartment()"
          ><span style="font-weight:bold;font-size:13px">+</span> Add
          department</vs-button
        >
        <hr />
        <vs-button
          color="dark"
...
// Store Module
import moduleHrReportManagement from "@/store/get-all-departments/moduleHrReportManagement.js";
  ...
  methods: {
    ...
    goToDeleteDepartment: function() {
     ...
     // alert(selectedDataStringPresentation);
        axios
        .delete(
          `${process.env.VUE_APP_BASE_URI}/dashboard/department/${selectedDataStringPresentation}/delete/`
        )
        .then(response => {
...
//////////////////////
    ...
    onRowDoubleClicked(event) {
      ...
      router.push("/Update/Department?SID="+event.data.id);
  ...
  mounted() {
    ...
    this.$store.dispatch("department/fetchItemsMainReport").then(response => {
      this.$vs.loading.close();
      if (response.data.status === 404) {
        this.$vs.notify({
          title: "Error",
  ...
  created() {
    if (!moduleHrReportManagement.isRegistered) {
      this.$store.registerModule("department", moduleHrReportManagement);
