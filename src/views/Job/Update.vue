  ...
  <div id="data-list">
    ...
    >
      <form>
        ...
        {{ category}} -->
        <vs-button
          :disabled="category == null || company == null || job_name == null || department == null"
          type="filled"
          @click="openConfirm()"
          class="mb-2"
          >UPDATE</vs-button
...
export default {
  ...
  methods: {
    ...
    updateJob() {
      ...
      axios
        .post(`${process.env.VUE_APP_BASE_URI}/dashboard/job/${this.$route.query.SID}/update/`, formData)
        .then(response => {
  ...
  mounted() {
    ...
    axios
      ...
      //  //console.log(response.data.data[0].department_name);
        ...
        axios
      .get(
        `${process.env.VUE_APP_BASE_URI}/dashboard/job/${this.$route.query.SID}/`
      )
      .then(response => {
        this.job_name = response.data.data.title;
        if(response.data.data.is_staff === false && response.data.data.is_TopManagment === false) this.category = 'Operation';
        else if(response.data.data.is_staff === true && response.data.data.is_TopManagment === true) this.category = 'Staff';
        else if(response.data.data.is_staff === false && response.data.data.is_TopManagment === true) this.category = 'Top Managment';
        else if(response.data.data.is_staff === true && response.data.data.is_TopManagment === false) this.category = 'Staff';
        this.department = response.data.data.departments_list.map(depts=>depts.department_name);  
      });
  },
  created() {
