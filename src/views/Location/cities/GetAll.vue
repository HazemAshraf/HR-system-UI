  ...
  <div id="data-list">
    <div class="vx-card p-6">
      <div class="flex flex-wrap items-center">
        ...
        <vs-button class="mb-4 md:mb-0" @click="goToAddCity()"
...
export default {
  ...
  methods: {
      ...
      this.$vs.dialog({
        ...
        accept:this.goToDeleteCity
      })
      },
    goToAddCity() {
      router.push("/location/add/cities");
    },
    goToDeleteCity() {
      this.$vs.loading();
      const selectedNodes = this.gridApi.getSelectedNodes();
     ...
     // alert(selectedDataStringPresentation);
        axios
        .delete(
          `${process.env.VUE_APP_BASE_URI}/dashboard/city/${selectedDataStringPresentation}/delete/`
        )
        .then(response => {
...
//////////////////////
    ...
    onRowDoubleClicked(event) {
      ...
      router.push("/location/update/cities?SID="+event.data.id);
  ...
  created() {
    if (!moduleHrReportManagement.isRegistered) {
      this.$store.registerModule("city", moduleHrReportManagement);
