  ...
  <div id="data-list">
    ...
    >
      <form>
        ...
        <vs-button :disabled="area_name == null || city == null || radius == null || radius < 0 || lng == null || lat == null" type="filled" @click="openConfirm()" class="mb-2"
          >UPDATE</vs-button
        >
      </form>
    </vx-card>
...
export default {
  ...
  methods: {
    ...
    updateArea() {
      ...
      axios
        .post(`${process.env.VUE_APP_BASE_URI}/dashboard/area/${this.$route.query.SID}/update/` , formData)
        .then(response => {
    ...
    //   .then(response => {
        ...
        .catch(error => {
         ...
         // reject(error);
           ...
           axios
      .get(
        `${process.env.VUE_APP_BASE_URI}/dashboard/area/${this.$route.query.SID}/`
      )
      .then(response => {
        this.area_name = response.data.data.name;
        this.city = response.data.data.city_info.name;
        this.lat = response.data.data.lat;
        this.lng = response.data.data.lng;
        this.radius = response.data.data.radius;
      });
