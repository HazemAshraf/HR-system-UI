  ...
  <div id="data-list">
    <div class="vx-card p-6">
      <div class="flex flex-wrap items-center">
        ...
        <vs-button class="mb-4 md:mb-0" @click="goToAddArea()"
...
// Store Module
import moduleHrReportManagement from "@/store/get-all-areas/moduleHrReportManagement.js";
...
export default {
  ...
  data() {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: "UNIT START TIME",
          field: "unit_start_time",
          sortable: true,
          filter: true,
          width: 180
        },
        {
          headerName: "UNIT END TIME",
          field: "unit_end_time",
          sortable: true,
          filter: true,
          width: 180
  ...
  computed: {
    ...
    //  //console.log("hahahahahah : " + this.$store.state.city.items);
      return this.$store.state.area.items;
    },
    paginationPageSize() {
  ...
  methods: {
      ...
      this.$vs.dialog({
        ...
        accept:this.goToDeleteArea
      })
      },
    goToAddArea() {
      router.push("/location/add/areas");
    },
    goToDeleteArea() {
      this.$vs.loading();
      const selectedNodes = this.gridApi.getSelectedNodes();
     ...
     // alert(selectedDataStringPresentation);
        axios
        .delete(
          `${process.env.VUE_APP_BASE_URI}/dashboard/area/${selectedDataStringPresentation}/delete/`
        )
        .then(response => {
...
//////////////////////
              ...
              this.$vs.notify({
                title: "Error",
                text: "Can't delete this company",
                iconPack: "feather",
                icon: "icon-alert-circle",
                color: "danger"
              });
    ...
    addNewFilter() {
      ...
      router.push("/location/update/areas?SID="+event.data.id);
  ...
  created() {
    if (!moduleHrReportManagement.isRegistered) {
      this.$store.registerModule("area", moduleHrReportManagement);
