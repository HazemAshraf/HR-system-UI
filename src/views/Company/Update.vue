        ...
        <div class="vx-row">
          <div class="vx-col md:w-1/2 sm:w-1/2 w-full mb-2">
            <label class="text-sm opacity-75"
              ><span class="text-primary">*</span>COMPANY NAME</label
            >
            <vs-input
              class="w-full"
              icon-pack="feather"
          ...
          <div class="vx-col md:w-1/2 sm:w-1/2 w-full mb-2">
            <label class="text-sm opacity-75"
              ><span class="text-primary">*</span>COMPANY CODE</label
            >
            <vs-input
              class="w-full"
              icon-pack="feather"
...
export default {
  ...
  data() {
    return {
      company_name: this.$route.query.SID,
      company_code: null,
      employee: {
        company: null,
        job_title: null
  ...
  methods: {
    openConfirm(){
      ...
      this.$vs.dialog({
        ...
        accept:this.updateCompany
      })
      },
    updateCompany() {
      this.$vs.loading();
         const formData = new FormData();
      formData.append("title", this.company_name);
      axios
        .post(
          `${process.env.VUE_APP_BASE_URI}/dashboard/company/${this.$route.query.SID}/update/` , formData
        )
        .then(response => {
  ...
  mounted() {
    ...
    axios
      .get(
        `${process.env.VUE_APP_BASE_URI}/dashboard/company/${this.$route.query.SID}/`
      )
      .then(response => {
        this.company_name = response.data.data.title;
      });
  },
  created() {
