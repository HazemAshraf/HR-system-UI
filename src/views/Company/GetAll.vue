  ...
  <div id="data-list">
    <div class="vx-card p-6">
      <div class="flex flex-wrap items-center">
        ...
        <vs-button class="mb-4 md:mb-0" @click="goToAddCompany()"
...
// Store Module
import moduleHrReportManagement from "@/store/get-all-companies/moduleHrReportManagement.js";
  ...
  methods: {
   openConfirm(){
      ...
      this.$vs.dialog({
        ...
        accept:this.goToDeleteCompany
      })
      },
    goToAddCompany() {
      router.push("/Add/Company");
    },
    goToDeleteCompany: function() {
      this.$vs.loading();
      const selectedNodes = this.gridApi.getSelectedNodes();
      const selectedData = selectedNodes.map(node => node.data);
      const selectedDataStringPresentation = selectedData
     ...
     // alert(selectedDataStringPresentation);
        axios
        .delete(
          `${process.env.VUE_APP_BASE_URI}/dashboard/company/${selectedDataStringPresentation}/delete/`
        )
        .then(response => {
...
//////////////////////
  ...
  mounted() {
    ...
    this.$store.dispatch("company/fetchItemsMainReport").then(response => {
      this.$vs.loading.close();
      if (response.data.status === 404) {
        this.$vs.notify({
          title: "Error",
  ...
  created() {
    if (!moduleHrReportManagement.isRegistered) {
      this.$store.registerModule("company", moduleHrReportManagement);
